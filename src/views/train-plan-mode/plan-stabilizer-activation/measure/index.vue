 ...
 * @LastEditTime: 2023-11-07 11:37:38
 * @Description : 方案-内核心激活训练-具体测量
-->
<template>
  <div
    class="plan-stabilizer-activation-measure"
        ...
        this.$axios
          ...
          .then(res => {
            ...
            if (data.status === 1) {
              /* 成功 */
              this.isFinished = true
              this.dataId = data.result.train_plan_record_id
              this.$message({
              ...
              /* 失败 */
              this.$alert(
                `[状态码为 ${data.status}] 数据保存失败，请点击"返回方案选择"按钮，然后重新测试！`,
                '警告',
                {
                  type: 'error',
                  showClose: false,
          ...
          .catch(err => {
            this.$alert(
              `[方案-内核心激活训练环节] ${err}。请确保网络连接正常，点击"返回方案选择"按钮，然后重新测试！`,
              '网络请求错误',
              {
                type: 'error',
                showClose: false,
